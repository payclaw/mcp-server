...
server.tool(
  ...
  "Get a PayClaw virtual card to make a purchase on behalf of the user. Returns card details and an intent_id. Call payclaw_reportPurchase after the transaction.",
  {
    ...
    description: z.string().describe("Brief description of what is being purchased"),
  },
...
server.tool(
  ...
  "Report the outcome of a purchase after using a PayClaw card from payclaw_getCard. Must be called after every purchase attempt.",
  {
    ...
    actual_amount: z.number().positive().optional().describe("Actual amount charged in USD"),
    merchant_name: z.string().optional().describe("Merchant name as it appeared on the receipt"),
    items: z.string().optional().describe("Items purchased (free-form description)"),
    order_confirmation: z.string().optional().describe("Order confirmation number or ID"),
  },
